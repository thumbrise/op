...
The distance between his work and ours is not a comparison. It is thirty years of road, paved by everyone in between. We stand at the far end. He stands at the beginning. The respect is not a paragraph — it is the entire section of this repository called [Dream](dreams/).
...
- **[Hystrix](https://github.com/Netflix/Hystrix)** (Netflix), **[resilience4j](https://resilience4j.readme.io/)**, and **[Polly](https://www.pollydocs.org/)** — circuit-breaker and resilience libraries referenced in [#1](universal/001-curl-eyeballs) as examples of retry/timeout/fallback patterns lifted into application code. Each proved at industrial scale that resilience can be expressed as a contract above transport — the same recognition Op generalises one floor up.
...
- **[Plug and Play](https://en.wikipedia.org/wiki/Plug_and_play)** (Microsoft, Intel, Compaq, others — early 1990s) and the **[VESA BIOS Extensions](https://en.wikipedia.org/wiki/VESA_BIOS_Extensions)** (VBE, 1989) — referenced in [#9](universal/009-the-address-is-a-detail) as the historical precedent for *devices that describe themselves so the system resolves addresses*. The hand-edited `config.sys` and IRQ-table era was real, painful, and ended only when a layer above it learned to ask devices what they were. Op is the same move, one floor up: programs describe themselves, compilers resolve URLs.
...
The fourteen chapters of [Dream](dreams/) illustrate Op's primitive thesis through historical, scientific, and artistic episodes. Each chapter cites real people and real work. They appear here because the rule is simple: if a name appears in the materials, it appears in this ledger.
