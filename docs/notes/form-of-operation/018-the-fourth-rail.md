...
> **Schema state — landed.** The changes described in this devlog — singular rail names (`error`, `trait`) and a unified four-rail structure with one Term format across all rails — are reflected in the current reference schemas: [`docs/reference/operation.v1.json`](../../reference/operation.v1.json) · [`docs/reference/instruction.v1.json`](../../reference/instruction.v1.json).
