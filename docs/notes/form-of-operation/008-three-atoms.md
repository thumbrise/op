...
We did not write a validator. We did not write a linter. We did not write an IDE plugin. We wrote JSON schema — [instruction.v1.json](/reference/instruction.v1.json) — and got all of that for free.
...
`op-sdk-go`: Instruction, Operation, Rail, Term. Go structs. Generated from [instruction.v1.json](/reference/instruction.v1.json) by `gojsonschema`. One command.
...
That is it. The entire protocol.

## Why GitHub Pages, Not a Custom Domain
The schema lives at `https://thumbrise.github.io/op/reference/instruction.v1.json`. Not on a custom domain. By choice.
...
- [The Schema Of Instruction](/reference/instruction.v1.json) — the form of instruction.
