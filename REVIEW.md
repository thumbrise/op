...
1. **[Dream](https://thumbrise.github.io/op/books/dream/)** — a short book with no technical words. Explains the idea through kitchens, taps, forests, termites, radio, DNA, bees, starlings, ports, and a black square. If you read nothing else, read this.
...
- **PR pipeline gates only what the author controls** — security scanning (govulncheck, audit) belongs in the release pipeline, not PR gates.
...
- **Absolute URLs where relative paths work.** `[REVIEW.md](REVIEW.md)` survives a repo rename. `https://github.com/org/repo/blob/main/REVIEW.md` does not.
...
- **Banned word: "generator" in the context of Op.** Never call Op tools generators — not in code, not in docs, not in devlogs, not in chat, not in commit messages. A generator produces something from nothing, by template, on demand. Op does none of this. Op carries instructions that already exist, and compilers translate them between forms. "Compiler", "reader", "emitter", "translator" — any of these fit. "Generator" implies Op fabricates behaviour, which inverts the whole protocol — the program already knows what it does, Op only moves that knowledge. The word is a lie and must not enter the glossary.
