...
## Usage - ONLY for NEW projects

```
/graphify .                           # full pipeline - ONLY for new project, first run
/graphify <path>                   # new project in specific folder
/graphify https://github.com/<owner>/<repo>  # clone and analyze new repo
/graphify query "<question>"          # read-only search - NEVER modifies
/graphify path "A" "B"              # read-only path finding
/graphify explain "X"              # read-only explanation
```

**DO NOT use on existing graph.json: --update, --cluster-only, --watch, --rebuild**
...
## Community Categorization (Portable - works with any project)

When adding nodes to graph.json, always assign semantic community:

**Rule: Read existing categories from current project!**

1. Read `graphify-out/graph.json` → find `communities` array (contains `{id, label}`)
2. Analyze each community's label to understand what it means
3. For new node, find best-fit by:
   - Matching keywords in node label with community label
   - Checking which community node's neighbors belong to
   - Using file_type as hint (code→Code Style, rationale→Concepts, etc.)
4. If no match → create new entry in `communities` array with meaningful label
5. Set node's `community` field to matching category `id`
6. Run `python3 graphify-out/validate.py` to verify

**Key principle:** For each new node, understand its meaning first, then see how it fits into the overall category design of this project. Don't use keyword matching - use semantic understanding and design coherence.

---

## GOLDEN RULES for EXISTING graph.json

**If graphify-out/graph.json already exists:**
1. **NEVER run** /graphify ., --update, --cluster-only, --watch, --rebuild
2. **ONLY manual edit** graph.json
3. **ONLY read operations**: query, path, explain
4. Run `python3 graphify-out/validate.py` to verify before changes

**The only files that matter:**
- graph.json ← source of truth (backend)
- graph.html ← reads graph.json dynamically (frontend)
- graphify-out/validate.py ← validation only

**What to delete/don't touch:**
- All auto-regeneration logic
- --update, --cluster-only, --watch flags
- merge logic
...
```json
{
  "nodes": [...],
  "links": [...],
  "communities": [{"id": 0, "label": "Category Name"}]
}
```

- `community` in nodes: for graph structure
- `communities` array: human-readable labels

**Key commands:**
- `python3 graphify-out/validate.py` - verify graph.json structure
- `python3 graphify-out/query.py --node NODE` - query node + memory
- `python3 graphify-out/add_memory.py "insight in English" --node NODE --tags tag1,tag2` - add memory (English, work-only)
- Refresh browser - graph.html loads latest graph.json

## Memory System (like traits in OP)
- graph.json = facts (operation core)
- memory/ = insights, opinions (traits attached to nodes)
- ALWAYS use add_memory.py (never edit memory/ manually)
- BLOCKS: personal content, non-English, invalid nodes

## Adding Memory
```bash
# Single addition (agent-safe)
python3 graphify-out/add_memory.py "insight in English" --node NODE --tags tag1,tag2

# Bulk addition (cheaper)
python3 graphify-out/bulk_memory.py "node1: insight 1 --tags t1,t2 | node2: insight 2"

# Add memory that supersedes old opinion (WAL-style)
python3 graphify-out/add_memory.py "Actually useful for X" --node NODE --tags tag1 --supersedes ins_20260505_154600

# Query with history (show superseded)
python3 graphify-out/query.py --node NODE --history

# Query latest only
python3 graphify-out/query.py --node NODE --latest

# Auto-detect conflicts
python3 graphify-out/query.py --node NODE --conflicts
```
